...
{
    [TestClass]
    public class JsonPaymentRepositoryTests
    {
        private string _testFilePath = null!;
        private JsonPaymentRepository _repository = null!;

        [TestInitialize]
        public void Setup()
        {
            _testFilePath = Path.Combine(Path.GetTempPath(), $"payments_test_{Guid.NewGuid()}.json");
            _repository = new JsonPaymentRepository(_testFilePath);
        }
        ...
        [TestMethod]
        public void Constructor_ShouldCreateFileIfNotExists()
        {
        ...
        [TestMethod]
        public async Task AddAsync_ShouldPersistToFile()
        {
            ...
            // Verify file actually contains data
            var fileContent = File.ReadAllText(_testFilePath);
            Assert.IsTrue(fileContent.Contains(payment.Id.ToString()));
        }
        ...
        [TestMethod]
        public async Task UpdateAsync_ShouldModifyAndPersist()
        {
            var payment = CreateTestPayment();
            await _repository.AddAsync(payment);

            payment.Amount = 500.50m;
            payment.Currency = "EUR";
            await _repository.UpdateAsync(payment);

            var updated = await _repository.GetByIdAsync(payment.Id);
            Assert.IsNotNull(updated);
            Assert.AreEqual(500.50m, updated.Amount);
            Assert.AreEqual("EUR", updated.Currency);
        }

        [TestMethod]
        public async Task DeleteAsync_ExistingId_ShouldRemoveAndPersist()
        {
            var payment = CreateTestPayment();
            await _repository.AddAsync(payment);
        ...
        [TestMethod]
        public async Task DataPersistsAcrossRepositoryInstances()
        {
            var payment = CreateTestPayment();
            await _repository.AddAsync(payment);

            // Create a new repository instance pointing to the same file
            var newRepository = new JsonPaymentRepository(_testFilePath);
